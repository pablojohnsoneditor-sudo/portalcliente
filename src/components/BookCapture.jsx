...
const TIPOS = ['Evento', 'Entrevista / Depoimento', 'Externo / Locação', 'Estúdio', 'Outro']

export default function BookCapture() {
  const ref = useRef(null)
  const { user } = useUser()
  const [form,   setForm]   = useState({ tipo: '', data: '', hora: '', local: '', obs: '' })
  ...
  const isValid = form.tipo && form.data && form.hora
  ...
  const submit = async () => {
    ...
    const { error } = await supabase.from('demands').insert({
      ...
      subtype:     form.tipo,
      details:     { tipo: form.tipo, data: form.data, hora: form.hora, local: form.local, obs: form.obs },
      status:      'pendente',
    })
  ...
  const reset = () => { setForm({ tipo: '', data: '', hora: '', local: '', obs: '' }); setStatus('idle') }

  return (
    <section ref={ref} className="flex flex-col gap-6">
      <div className="reveal">
        <p className="text-[10px] font-bold tracking-[.2em] uppercase text-lime mb-1">Agendar</p>
        ...
        <p className="text-snow/50 text-sm mt-2 leading-relaxed">
          Preencha os detalhes da gravação. Pablo confirma em até 2h.
        </p>
      </div>
      ...
      {status === 'saved' ? (
        <div className="reveal bg-card border border-lime/30 p-10 text-center flex flex-col items-center gap-4">
          ...
          <div>
            ...
            <p className="text-snow/50 text-sm mb-4">Pablo vai confirmar o agendamento em breve.</p>
        ...
        <div className="flex flex-col gap-4">
          ...
          <div className="reveal flex flex-col gap-1.5">
            <label className="text-[10px] font-bold tracking-widest uppercase text-snow/60">Tipo de captação *</label>
          ...
          <div className="reveal grid grid-cols-2 gap-3">
            <div className="flex flex-col gap-1.5">
              <label className="text-[10px] font-bold tracking-widest uppercase text-snow/60">Data *</label>
              <input type="date" value={form.data} onChange={set('data')} className="inp" />
            </div>
            <div className="flex flex-col gap-1.5">
              <label className="text-[10px] font-bold tracking-widest uppercase text-snow/60">Horário *</label>
              <input type="time" value={form.hora} onChange={set('hora')} className="inp" />
            </div>
          </div>

          <div className="reveal flex flex-col gap-1.5">
            <label className="text-[10px] font-bold tracking-widest uppercase text-snow/60">Local / Endereço</label>
            <input value={form.local} onChange={set('local')} className="inp" placeholder="Endereço ou nome do local" />
          </div>

          <div className="reveal flex flex-col gap-1.5">
            <label className="text-[10px] font-bold tracking-widest uppercase text-snow/60">Observações</label>
            <textarea value={form.obs} onChange={set('obs')} rows={3} className="inp resize-none"
              placeholder="Referências, detalhes, equipamento necessário..." />
          </div>
