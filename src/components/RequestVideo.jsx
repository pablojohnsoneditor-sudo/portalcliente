...
const TIPOS = ['Reels / Short', 'Vídeo institucional', 'Documentário', 'Outro']

export default function RequestVideo() {
  const ref = useRef(null)
  const { user } = useUser()
  const [form,   setForm]   = useState({ tipo: '', gravado: '', prazo: '', desc: '' })
  ...
  const buildWAMsg = () => encodeURIComponent(
    ...
    `*Nome:* ${user?.name}\n` +
    `*Tipo:* ${form.tipo}\n` +
    `*Material gravado:* ${form.gravado || 'Não informado'}\n` +
  ...
  const submit = async () => {
    ...
    const { error } = await supabase.from('demands').insert({
      ...
      subtype:     form.tipo,
      details:     { tipo: form.tipo, gravado: form.gravado, prazo: form.prazo, desc: form.desc },
      status:      'pendente',
    })
  ...
  const reset = () => { setForm({ tipo: '', gravado: '', prazo: '', desc: '' }); setStatus('idle') }
    ...
    <section ref={ref} className="flex flex-col gap-6">
      <div className="reveal">
        ...
        <p className="text-snow/50 text-sm mt-2 leading-relaxed">
          Descreva o vídeo que precisa. Pablo entra em contato para alinhar os detalhes.
        </p>
      </div>
      ...
      {status === 'saved' ? (
        <div className="reveal bg-card border border-lime/30 p-10 text-center flex flex-col items-center gap-4">
          ...
          <div>
            ...
            <p className="text-snow/50 text-sm mb-4">Pablo entra em contato em breve para alinhar o projeto.</p>
        ...
        <div className="flex flex-col gap-4">
          ...
          <div className="reveal flex flex-col gap-1.5">
            <label className="text-[10px] font-bold tracking-widest uppercase text-snow/60">Tipo de vídeo *</label>
          ...
          <div className="reveal flex flex-col gap-1.5">
            <label className="text-[10px] font-bold tracking-widest uppercase text-snow/60">Material já gravado?</label>
            <div className="grid grid-cols-3 gap-2">
              {['Sim, tenho tudo', 'Tenho parcialmente', 'Ainda não gravei'].map(o => (
                <button key={o} type="button" onClick={() => setForm(f => ({ ...f, gravado: o }))}
                  className={`p-3 border text-left text-xs font-bold transition-all duration-150
                    ${form.gravado === o
                      ? 'border-lime bg-lime/5 text-lime'
                      : 'border-border bg-card text-snow/60 hover:border-snow/20 hover:text-snow'}`}>
          ...
          <div className="reveal flex flex-col gap-1.5">
            ...
            <textarea value={form.desc} onChange={set('desc')} rows={4} className="inp resize-none"
              placeholder="Conte sobre o projeto, estilo desejado, referências..." />
          </div>
